    ...
    int* delayb)
{
    ...
    for( int i=tid; i<num; i+=tot_threads ){
        ...
        if(npre[i]){
            ...
            if( spike[pre] )
                new_a[1] += ar*ad;
                //new_a[1] += ar*ad * expf(-1.0 * dopamine[i]);
            new_a[2] = -( ar+ad )*old_a[1] - ar*ad*old_a[0];
            // new_a[2] = -( ar+ad )*old_a[1] - ar*ad*old_a[0] * dopamine[i];
        ...
        if(npreb[i]){
            ...
            if( spikeb[pre] )
                dopamine[i] += 1.0 * fmax( 0., new_a[0]*gmax) - 0.000005;
                //dopamine[i] += 100000.0 * fmax( 0., new_a[0]*gmax) - 0.05;
		}
        dopamine[i] = dopamine[i] - 0.0005 * dopamine[i]; // Dopamine decay
}
